...
int main()
{
    ...
    int tempo_de_espera = 0, tempo_de_resposta = 0, flag_retirou_anteriormente =0;
    ...
    for(i = 0; x != 0; i++)
    {
        ...
        {
            if (flag_retirou_anteriormente != 1){
                processo[lista_prioridade_alta[0]-1].tempo_processamento++;
            }
            else{
               processo[lista_prioridade_alta[0]-1].tempo_processamento = processo[lista_prioridade_alta[0]-1].tempo_processamento; 
            }
        ...
        //se nao existir processo na alta, verifica lista de prontos de baixa prioridade
                ...
                {
                    if (flag_retirou_anteriormente != 1){
                        processo[lista_prioridade_baixa[0]-1].tempo_processamento++;
                    }
                    else{
                        processo[lista_prioridade_baixa[0]-1].tempo_processamento = processo[lista_prioridade_baixa[0]-1].tempo_processamento;
                    }
                    //percorre a lista de tempos io do processo atual e compara com o tempo atual de execucao 
        ...
        if( (i!= 0) && (i%quantum == 0) )
        {
            ...
            if( lista_prioridade_alta[0] != 0 )
            {
                printf("Sou o processo [%d] e estou na fila ALTA\n",lista_prioridade_alta[0]);
                if (flag_retirou_anteriormente == 1){
                    flag_retirou_anteriormente = 0;
                    printf("\nProcesso terminado anteriormente, não preemptar.\n");
                }
                else{
            ...
            {
                printf("Sou o processo [%d] e estou na fila BAIXA\n",lista_prioridade_baixa[0]);
                //funcao para inserir o processo atual no final da fila de baixa prioridade
                if (flag_retirou_anteriormente == 1){
                    flag_retirou_anteriormente = 0;
                    printf("\nProcesso terminado anteriormente, não preemptar.\n");
        ...
        if(lista_prioridade_alta[0] != 0)
        {
            printf("\nVERIFICANDO ALTA: sou o processo [%d], tempo de processamento: [%d], tempo explosao: [%d]", lista_prioridade_alta[0], processo[lista_prioridade_alta[0]-1].tempo_processamento, processo[lista_prioridade_alta[0]-1].tempo_de_explosao_processo);
            getchar();
        ...
        {
            printf("\nVERIFICANDO BAIXA: sou o processo [%d], tempo de processamento: [%d], tempo explosao: [%d]", lista_prioridade_baixa[0], processo[lista_prioridade_baixa[0]-1].tempo_processamento, processo[lista_prioridade_baixa[0]-1].tempo_de_explosao_processo);
            getchar();
            if(processo[lista_prioridade_baixa[0]-1].tempo_processamento == processo[lista_prioridade_baixa[0]-1].tempo_de_explosao_processo)
            {
                ...
                for (j = 1 ; j < qtd_total_processos ; j++){
                        printf("\nMudando o processo com id [%d] para a posicao %d\n", lista_prioridade_baixa[j], j-1);
                        lista_prioridade_baixa[j-1]=lista_prioridade_baixa[j];
